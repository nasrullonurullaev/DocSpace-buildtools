...
RUN echo "--- build/publish docspace-server .net 10.0 ---" && \
    ...
    dotnet build ASC.Migrations.sln -c Release -r linux-x64 -m:1 --self-contained false --property:OutputPath=${SRC_PATH}/publish/services/ASC.Migration.Runner/service/ -p:DebugType=None -p:DebugSymbols=false -p:UseAppHost=false --no-incremental && \
